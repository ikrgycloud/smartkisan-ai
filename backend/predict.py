...
MODEL_PATH = (
    ...
    / "model"
    / "crop_prediction_model_balanced.pkl"
)
...
def get_advice(crop: str) -> str:
    return CROP_ADVICE.get(
        crop,
        "Consult a local agricultural expert for best practices."
    )
...
def run_prediction(data) -> dict:
    if model is None:
        raise RuntimeError(
            f"ML model not loaded. Expected model at: {MODEL_PATH}"
        )
    ...
    # Approved Step 62 rule:
    # Pulses -> Mustard
    # Mustard probability boost = 1.10
    adjusted_probabilities = probabilities.copy()
    classes = list(model.classes_)

    if "Pulses" in classes and "Mustard" in classes:
        mustard_index = classes.index("Mustard")
        adjusted_probabilities[mustard_index] *= 1.10

    final_index = adjusted_probabilities.argmax()
    prediction = model.classes_[final_index]

    top_indices = adjusted_probabilities.argsort()[-3:][::-1]

    top3 = [
        {
            "crop": str(model.classes_[i]),
            "confidence": round(
                float(adjusted_probabilities[i] * 100),
                2
            )
        }
    ...
    adjusted_max_probability = adjusted_probabilities.max()

    confidence = round(
        float(adjusted_probabilities[final_index] * 100),
        2
    )

    if adjusted_probabilities[final_index] != adjusted_max_probability:
        raise RuntimeError(
            "Adjusted probability confidence mismatch."
        )

    return {
        "top_crop": str(prediction),
        "confidence": confidence,
        "top3": top3,
        "advice": get_advice(str(prediction)),
    }
